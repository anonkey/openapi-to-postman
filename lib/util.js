  ...
  */
    ...
    if (type === 'root' || type === 'method') {
      ...
      _.forEach(commonPathVars, (variable) => {
        variables.push({
          ...
          value: variable.schema.example || this.options.schemaFaker ?
  ...
  convertToPmBodyData: function(bodyObj, contentType, bodyTypeOption, indentCharacter = '') {
    ...
    if (bodyObj.example) {
      ...
      if (this.options.schemaFaker) {
        ...
        bodyData = bodyObj.schema.example || safeSchemaFaker(bodyObj.schema || {}, bodyTypeOption, this.components, schemaType, indentCharacter);
      }
      else {
  ...
  convertToPmQueryParameters: function(param) {
    ...
    if (param.hasOwnProperty('schema')) {
      // fake data generated
      paramValue = param.schema.example || this.options.schemaFaker ?
  ...
  convertToPmHeader: function(header) {
    ...
    if (header.hasOwnProperty('schema')) {
      if (!this.options.schemaFaker) {
        ...
        fakeData = header.schema.example || safeSchemaFaker(header.schema || {}, this.components, SCHEMA_FORMATS.DEFAULT);

        // for schema.type=string or number,
        // adding JSON.stringify will add unnecessary extra double quotes around the value
        if (!header.schema.example && header.schema && (header.schema.type === 'object')) {
